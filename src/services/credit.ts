...
export async function getUserCredits(user_uuid: string): Promise<UserCredits> {
  ...
  try {
    ...
    // Get all credits (both valid and expired/used) for total calculation
    const allCredits = await getAllUserCredits(user_uuid);
    let totalEarnedCredits = 0;
    
    if (allCredits) {
      allCredits.forEach((v) => {
        if (v.credits > 0) {
          totalEarnedCredits += v.credits;
        }
      });
    }
    user_credits.total_credits = totalEarnedCredits;
    ...
    user_credits.used_credits = Math.max(0, totalEarnedCredits - user_credits.left_credits);
